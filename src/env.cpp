...
DemoEnv::DemoEnv(std::string& urdf_path) {
  ...
  for (int i = 1; i < rmt_joints_axis.size(); ++i) {
    ...
    arm_joint_init_angles_.emplace_back(0.);
  }
...
void DemoEnv::main() {
  ...
  Eigen::Vector3d translation{0.8, 0.2, 0.4};
  ...
  // std::endl; TransMat testing_ee_pose =
  // kinematic_chain_->forwardKinematic(origin_q); std::cout << "The testing EE
  // position in world frame is: "
  //           << testing_ee_pose.pos.transpose() << std::endl;
  // std::cout << "The testing EE rotation in world frame is: \n"
  //           << testing_ee_pose.rot << std::endl;
  /////////////////////////////////////////////////////////////////////
  // jacobian
  /////////////////////////////////////////////////////////////////////
  Eigen::MatrixXd jacobian_mat_B(6, robotic_model.nv), testing_jacobian_B;
  ...
  testing_jacobian_B = kinematic_chain_->calculateBodyJacobian(origin_q);
  ...
  kinematic_chain_->inverseKinematic(rot, translation, origin_q,
                                     testing_desired_theta, testing_error);
  end_time = std::chrono::system_clock::now();
  duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time -
                                                                   start_time);
  std::cout << "["
